...
#ifdef ENABLE_REFER
/* added by windinsn, Jan 28, 2012, ¼ì²éÊÇ·ñÓÐ @»ò»Ø¸´ÌáÐÑ */
     else if (chkmailflag==3) {
         prints("\033[5m");
         sprintf(genbuf, "[ÄúÓÐ@ÌáÐÑ]");
     } else if (chkmailflag==4) {
         prints("\033[5m");
         sprintf(genbuf, "[ÄúÓÐ»Ø¸´ÌáÐÑ]");
     }
#ifdef ENABLE_REFER_LIKE
     else if (chkmailflag==6) {
         prints("\033[5m");
         sprintf(genbuf, "[ÄúÓÐLikeÌáÐÑ]");
     }
#endif
#endif /* ENABLE_REFER */
...
int nomsg; /* fancyrabbit Oct 12 2007, nomsg == true ´Ó²»³öÏÖÌáÊ¾¸ÄÎª(²»³öÏÖÌáÊ¾ && ²»¼Æ bmlog) */
           /* jiangjun, 20120522, nomsg == true Ê±£¬°æÖ÷b4²Ù×÷, ²»½øÐÐboard_security_report */
    ...
    if (ann_get_path(key, buf, sizeof(buf)) == 0) {
        ...
        */
        ...
        if (a_savenames(&pm) == 0) {
            ...
            if (ret==0 && !nomsg) { /* °æÖ÷b4²Ù×÷²»½øÐÐboard_security_report */
                char filename[STRLEN];
                FILE *fn;
                gettmpfilename(filename, "import_post");
                if ((fn=fopen(filename, "w"))!=NULL) {
                    fprintf(fn, "\033[33mÊÕÂ¼¾«»ªÇøÄ¿Â¼: \033[4;32m%s\033[m\n", pm.path+17);
                    fclose(fn);
                }
                board_security_report(filename, getCurrentUser(), "IÊÕÂ¼", currboard->filename, fileinfo);
                unlink(filename);
            }
#endif
        } else {
...
int mode;
{
    ...
    if (!valid_fname(fname)) {
        ...
        if (boardname[0]) { /* ½«¼ÇÂ¼Ìí¼ÓÖÁ¶ÔÓ¦µÄ°æÃæ */
            if ((fn=fopen(filename, "w"))!=NULL) {
                fprintf(fn, "\033[33m%s±êÌâ: \033[4;32m%s\033[m\n", mode==ADDGROUP?"Ä¿Â¼":"ÎÄ¼þ", title);
                fprintf(fn, "\033[33m%sÃû³Æ: \033[4;32m%s\033[m\n", mode==ADDGROUP?"Ä¿Â¼":"ÎÄ¼þ", fname);
                fprintf(fn, "\033[33mÌí¼ÓÂ·¾¶: \033[4;32m%s\033[m\n", pm->path+17);
                fclose(fn);
            }
            sprintf(rtitle, "Ìí¼Ó¾«»ªÇø%s <%s>", mode==ADDGROUP?"Ä¿Â¼":"ÎÄ¼þ", title);
...
#undef ACP_DUMPS
    ...
    if ((type==PASTE_CUT && bname_s[0]) || bname_d[0]) { /* ½«¼ÇÂ¼Ìí¼ÓÖÁ¶ÔÓ¦µÄ°æÃæ */
        if ((fn=fopen(fname, "w"))!=NULL) {
            ...
            fprintf(fn, "\033[33m%s±êÌâ: \033[4;32m%s\033[m\n", S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼þ", title);
            fprintf(fn, "\033[33m%sÃû³Æ: \033[4;32m%s\033[m\n", S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼þ", filename);
            if (type==PASTE_COPY)
                if ((p=strrchr(path, '/')))
                    *p='\0';
            fprintf(fn, "\033[33mÔ­Ê¼Â·¾¶: \033[4;32m%s/\033[m\n", path+17);
            fprintf(fn, "\033[33mÄ¿±êÂ·¾¶: \033[4;32m%s/\033[m\n", pm->path+17);
            fclose(fn);
        }
        sprintf(rtitle, "%s¾«»ªÇø%s <%s>", type==PASTE_COPY?"¸´ÖÆ":"ÒÆ¶¯", S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼þ", title);
        if (type==PASTE_CUT) {
            if (bname_s[0]) {
...
/* jiangjun, 2012.02.05, [¾«»ªÇø] Çø¶Î¼ôÇÐ/¸´ÖÆ/Õ³Ìù²Ù×÷, ²Î¿¼etnlegendµÄ¾«»ªÇø¹¦ÄÜ */
...
#define ACP_ANY_RETURN(msg) do{clrtoeol();prints("\033[1;37m%s\033[0;33m<Any>\033[m",(msg));igetkey();pm->page=9999;return;}while(0)
    ...
    if (mode==0||mode==1) {
        int begin, end;
        snprintf(genbuf, STRLEN, "ÊäÈëÇø¶Î%sÆðÊ¼ÎÄÕÂÐòºÅ: ", mode==0?"¸´ÖÆ":"¼ôÇÐ");
        getdata(t_lines - 1, 0, genbuf, ans, 4, DOECHO, NULL, true);
        begin = atoi(ans);
        if (begin<=0 || begin>pm->num)
            ACP_ANY_RETURN("´íÎóµÄÎÄÕÂºÅ!!! ");
        getdata(t_lines - 1, 40, "ÊäÈëÖÕÖ¹ÎÄÕÂÐòºÅ: ", ans, 4, DOECHO, NULL, true);
        end = atoi(ans);
        if (end > pm->num || begin >= end)
            ACP_ANY_RETURN("´íÎóµÄÎÄÕÂºÅ!!! ");
        count = end-begin+1;
        if ((fp=fopen(filebuf, "w"))==NULL)
            ACP_ANY_RETURN("ÎÄ¼þ¶ÁÈ¡´íÎó");
        fprintf(fp, "%d\n%d\n", mode, count);
        ...
        fclose(fp);
        snprintf(genbuf, STRLEN, "Íê³É\033[32m%d\033[mÆªÎÄÕÂÇø¶Î%s±êÊ¶, µ«ÔÚÕ³Ìùºó·½¿ÉÉ¾³ýÔ´ÎÄ¼þ»òÄ¿Â¼!", count, (!mode)?"¸´ÖÆ":"¼ôÇÐ");
        ACP_ANY_RETURN(genbuf);
    }

    if ((fp=fopen(filebuf,"r"))==NULL)
        ACP_ANY_RETURN("Ê¹ÓÃÇø¶ÎÕ³ÌùÃüÁîÇ°Ó¦ÏÈÊ¹ÓÃÇø¶Î¼ôÇÐ»òÇø¶Î¸´ÖÆÃüÁî...");
...
#define ACP_ANY_CONTINUE(msg) {prints("\033[1;37m%s\033[0;33m<Any>\033[m",(msg));ch=igetkey();pm->page=9999;continue;}
    ...
    if (type==PASTE_ERROR) {
        fclose(fp);
        ACP_ANY_RETURN("Çø¶Î±êÊ¶´íÎó£¬ÇëÖØÐÂ½øÐÐÇø¶Î¼ôÇÐ»ò¸´ÖÆ²Ù×÷...");
    }
    snprintf(genbuf, STRLEN, "È·¶¨Çø¶Î%s²¢Õ³Ìù?(Y/N) [N]", type==PASTE_COPY?"¸´ÖÆ":"¼ôÇÐ");
    ...
    fn = fopen(fname, "w");
    if (fn) {
        fprintf(fn, "\033[45;33m¾«»ªÇøÇø¶Î%sµµ°¸ÁÐ±í\033[K\033[m\n", type==PASTE_COPY?"¸´ÖÆ":"ÒÆ¶¯");
        fprintf(fn, "\033[44mÐòºÅ [Àà±ð] µµ°¸±êÌâ                               µµ°¸Ãû³Æ\033[K\033[m\n");
    }
#endif
    do {
        if (toupper(ch)=='Q')
        ...
        if (!((access(newpath,F_OK)==-1)&&(errno==ENOENT))) {
            move(t_lines-1, 0);clrtoeol();
            snprintf(genbuf, STRLEN, "µ±Ç°Â·¾¶ÏÂÒÑ´æÔÚÃûÎª\033[32m%s\033[mÎÄ¼þ»òÄ¿Â¼, \033[31mQ\033[mÖÕÖ¹²Ù×÷, ÆäËûÈÎÒâ¼ü¼ÌÐø", filename);
            ACP_ANY_CONTINUE(genbuf);
        }
        if (!strncmp(path,newpath,(len=strlen(path)))&&newpath[len]=='/') {
            move(t_lines-1, 0);clrtoeol();
            ACP_ANY_BREAK("µ±Ç°²Ù×÷¼ôÇÐ»ò¸´ÖÆÄ¿Â¼ÖÁÆä×ÓÄ¿Â¼ÖÐ, ½«µ¼ÖÂËÀÑ­»·, ²Ù×÷ÖÕÖ¹");
        }
        if (stat(path,&st)==-1||!(S_ISDIR(st.st_mode)||S_ISREG(st.st_mode))) {
            move(t_lines-1, 0);clrtoeol();
            snprintf(genbuf, STRLEN, "Ô´ÎÄ¼þ»òÄ¿Â¼%s²»´æÔÚ, °´\033[31mQ\033[mÖÕÖ¹²Ù×÷, ÆäËûÈÎÒâ¼ü¼ÌÐø", filename);
            ACP_ANY_CONTINUE(genbuf);
        }
        ...
        if (!(type==PASTE_CUT&&!rename(path,newpath))) {
            if (S_ISDIR(st.st_mode)) {
                ...
                if (system(genbuf)) {
                    move(t_lines-1, 0);clrtoeol();
                    ACP_ANY_BREAK("¸´ÖÆÄ¿Â¼¹ý³ÌÖÐ·¢Éú´íÎó...");
                }
            } else {
                if (f_cp(path,newpath,0)) {
                    move(t_lines-1, 0);clrtoeol();
                    ACP_ANY_BREAK("¸´ÖÆÎÄ¼þ¹ý³ÌÖÐ·¢Éú´íÎó...");
                }
            }
            copy=1;
        }
        ...
        if (type==PASTE_CUT) {
            ...
            if (ret) {
                move(t_lines-1, 0);clrtoeol();
                ACP_ANY_BREAK("¼ôÇÐÎÄ¼þ¹ý³ÌÖÐ·¢Éú´íÎó...");
            }
        }
#ifdef BOARD_SECURITY_LOG
        if (fn) {
            ...
            fprintf(fn, "%4d [%s] %-38s %-s\n", i+1, S_ISDIR(st.st_mode)?"Ä¿Â¼":"\033[36mÎÄ¼þ\033[m", title, filename);
        }
#endif
        i++;
    } while(i<count);
    ...
    if (i) {
        move(t_lines-1, 0);clrtoeol();
        snprintf(genbuf, STRLEN, "Íê³É%dÆªÎÄÕÂµÄÇø¶Î%s²Ù×÷", i, (type==PASTE_COPY)?"¸´ÖÆ":"¼ôÇÐ");
#ifdef BOARD_SECURITY_LOG
        if (fn) {
            bname_s[0] = '\0';
            bname_d[0] = '\0';
            ...
            if ((type==PASTE_CUT && bname_s[0]) || bname_d[0]) { /* ½«¼ÇÂ¼Ìí¼ÓÖÁ¶ÔÓ¦µÄ°æÃæ */
                ...
                fprintf(fn, "\033[33mÄ¿±êÂ·¾¶: \033[4;32m%s/\033[m\n", pm->path+17);
                fclose(fn);
                sprintf(rtitle, "Çø¶Î%s¾«»ªÇøµµ°¸", (type==PASTE_COPY)?"¸´ÖÆ":"¼ôÇÐ");
                if (type==PASTE_CUT) {
                    if (bname_s[0]) {
                        board_security_report(fname, getCurrentUser(), rtitle, bname_s, NULL);
                        if (bname_d[0] && strcmp(bname_s, bname_d))
...
void a_delete(MENU *pm)
{
    ...
    if (a_savenames(pm)) {
        ...
        if (!S_ISLNK(st.st_mode) && (fn=fopen(filename, "w"))!=NULL) {
            fprintf(fn, "\033[33mµµ°¸Â·¾¶: \033[4;32m%s\033[m\n", path+10);
            fclose(fn);
            ...
            if (board[0])
                ...
                sprintf(buf, "ÏµÍ³");
            sprintf(title, "É¾³ý%s¾«»ªÇø%s <%s>", buf, S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼þ", anntitle);
...
#ifdef BOARD_SECURITY_LOG
        if (board[0]) { /* ½«¼ÇÂ¼Ìí¼ÓÖÁ¶ÔÓ¦µÄ°æÃæ */
            if ((fn=fopen(filename, "w"))!=NULL) {
                fprintf(fn, "\033[33mµµ°¸Â·¾¶: \033[4;32m%s\033[m\n", path+17);
                fclose(fn);
            }
            sprintf(title, "É¾³ý¾«»ªÇø%s <%s>", S_ISLNK(st.st_mode)?"Á´½Ó":(S_ISDIR(st.st_mode)?"Ä¿Â¼":"ÎÄ¼þ"), anntitle);
    ...
    if (!valid_fname(fname)) {
        ...
        if (a_savenames(pm) == 0) {
            if (f_mv(genbuf, fpath) == 0) {
                ...
                if (board[0]) { /* ½«¼ÇÂ¼Ìí¼ÓÖÁ¶ÔÓ¦µÄ°æÃæ */
                    if ((fn=fopen(filename, "w"))!=NULL) {
                        fprintf(fn, "\033[33mµµ°¸Ãû³Æ: \033[31m%s\033[m -> \033[32m%s\033[m\n", originname, fname);
                        fprintf(fn, "\033[33mµµ°¸Â·¾¶: \033[4;32m%s/%s\033[m\n", pm->path+17, fname);
                        fclose(fn);
                    }
                    strncpy(buf, M_ITEM(pm,pm->now)->title, 39);
                    buf[38] = '\0';
                    remove_blank_ctrlchar(buf, buf, false, true, true);
                    sprintf(title, "ÐÞ¸Ä¾«»ªÇø%sÃû³Æ <%s>", dashf(fpath)?"ÎÄ¼þ":"Ä¿Â¼", buf);
                    board_security_report(filename, getCurrentUser(), title, board, NULL);
                    unlink(filename);
                }
#endif
                ...
                if (item) {
                    ...
                    if (*changed_T && strcmp(unchanged_T, changed_T)) {
                        ...
                        if (a_savenames(pm) != 0) {
                            ...
                            if (board[0]) { /* ½«¼ÇÂ¼Ìí¼ÓÖÁ¶ÔÓ¦µÄ°æÃæ */
                                if ((fn=fopen(filename, "w"))!=NULL) {
                                    fprintf(fn, "\033[33mµµ°¸±êÌâ: \033[31m%s\033[m -> \033[32m%s\033[m\n", unchanged_T, changed_T);
                                    fprintf(fn, "\033[33mµµ°¸Â·¾¶: \033[4;32m%s\033[m\n", fpath+17);
                                    fclose(fn);
                                }
                                sprintf(title, "ÐÞ¸Ä¾«»ªÇø%s±êÌâ <%s>", dashf(fpath)?"ÎÄ¼þ":"Ä¿Â¼", changed_T);
                                board_security_report(filename, getCurrentUser(), title, board, NULL);
                                unlink(filename);
                            }
#endif
                ...
                if (me.now<me.num) {
                    do {
                        ...
                        if (!member_post_perm(bh, getCurrentUser())) {
                            prints("\n\n    %s\033[0;33m<enter>\033[m", "Ä¿µÄ°æÃæÎª×¤°æ¿ÉÐ´°æÃæ£¬·Ç×¤°æÓÃ»§²»ÄÜ×ªÔØÎÄÕÂÖÁ¸Ã°æ...");
                            WAIT_RETURN;
                            break;
                        }
#endif /* ENABLE_BOARD_MEMBER */
