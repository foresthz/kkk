...
/* jiangjun, 2012.02.05, [¾«»ªÇø] Çø¶Î¼ôÇÐ/¸´ÖÆ/Õ³Ìù²Ù×÷, ²Î¿¼etnlengentµÄ¾«»ªÇø¹¦ÄÜ */
...
#define ACP_ANY_CONTINUE(msg) {prints("\033[1;37m%s\033[0;33m<Any>\033[m",(msg));igetkey();pm->page=9999;continue;}
    ...
    if (type==PASTE_ERROR) {
        fclose(fp);
        ACP_ANY_RETURN("Çø¶Î±êÊ¶´íÎó£¬ÇëÖØÐÂ½øÐÐÇø¶Î¼ôÇÐ»ò¸´ÖÆÃüÁî...");
    }
    ...
    do {
        ...
        if (!((access(newpath,F_OK)==-1)&&(errno==ENOENT))) {
            move(t_lines-1, 0);clrtoeol();
            snprintf(genbuf, STRLEN, "µ±Ç°Â·¾¶ÏÂÒÑ´æÔÚÃûÎª\033[32m%s\033[mÎÄ¼þ»òÄ¿Â¼, ÈÎÒâ¼ü¼ÌÐø", filename);
        ...
        if (stat(path,&st)==-1||!(S_ISDIR(st.st_mode)||S_ISREG(st.st_mode))) {
            move(t_lines-1, 0);clrtoeol();
            snprintf(genbuf, STRLEN, "Ô´ÎÄ¼þ»òÄ¿Â¼%s²»´æÔÚ, ÈÎÒâ¼ü¼ÌÐø", filename);
            ACP_ANY_CONTINUE(genbuf);
        }
