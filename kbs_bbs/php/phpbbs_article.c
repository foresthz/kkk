...
void bbs_make_article_array(zval * array, struct fileheader *fh, char *flags, size_t flags_len)
{
    ...
    add_assoc_long(array, "ACCESSED0", fh->accessed[0] & 0xff);
    add_assoc_long(array, "ACCESSED1", fh->accessed[1] & 0xff);
...
void make_article_flag_array(char flags[5], struct fileheader *ent, struct userec *user, const struct boardheader *bh, int is_bm)
{
    flags[0] = get_article_flag(ent, user, bh, is_bm, &(flags[4]), getSession());
    ...
    */
    ...
    for (i = 0; i < record_cnt; i++) {
        ...
        if (articles[i].id && getCurrentUser()) {
            make_article_flag_array(flags, articles + i, getCurrentUser(), bp, is_bm);
        } else {
        ...
        if (i == 0 && ins_top && (ret_flag || articles[i].id != articles[i].groupid)) {
            ...
            for (j = bs->toptitle - 1; j >= 0; j--) {
                if (bs->topfh[j].id == bs->topfh[j].groupid && bs->topfh[j].id == gid) {
                    if (getCurrentUser())
                        make_article_flag_array(flags, bs->topfh + j, getCurrentUser(), bp, is_bm);
                    else
                        memset(flags, 0, sizeof(flags));
